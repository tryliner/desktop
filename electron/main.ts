...
function createWindow() {
  ...
  mainWindow.webContents.on("unresponsive", () => {
    console.warn("[Renderer Unresponsive] Window is temporarily unresponsive");
  });
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.handle("signer:sign-cover-url", (_event, payload: string) => {
      try {
        ...
        console.error("signCoverUrl error:", err);
        return payload;
      }
    });
    ...
    ipcMain.handle(
      ...
      (_event, request: SignRequestInput) => {
        try {
          ...
          console.error("signMonitorRequest error:", err);
          return { error: String(err?.message || err) };
        }
      },
    );
    ...
    ipcMain.handle(
      ...
      (_event, payload: number[] | Uint8Array) => {
        try {
          ...
          console.error("signRawPayload error:", err);
          return "";
        }
      },
    );
